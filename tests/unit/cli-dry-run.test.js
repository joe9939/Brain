...
module.exports = {
  ...
  run: () => {
    ...
    results.push({ name: 'dry-run exit 0', pass: r1.status === 0 });
    results.push({ name: 'dry-run shows checks', pass: r1.stdout.includes('✓') || r1.stdout.includes('checks') });
    results.push({ name: 'dry-run says pass', pass: r1.stdout.includes('PASSED') || r1.stdout.includes('ALL') });

    const r2 = spawnSync('node', [INSTALL, '--verify'], { cwd: path.join(__dirname, '../..'), encoding: 'utf8' });
    results.push({ name: '--verify exit 0', pass: r2.status === 0 });
