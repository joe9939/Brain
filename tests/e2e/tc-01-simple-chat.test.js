...
// Verifies brain-master.md fires L1 4-way parallel on every message
const fs = require('fs');
...
module.exports = {
  ...
  run: async () => {
    ...
    results.push({ name: 'L1 fires on every message', pass: content.includes('every message') || content.includes('Every message') });
    ...
    // All 4 agents present in prompt
    const agents = ['brain-thalamus', 'brain-amygdala', 'brain-hippocampus', 'brain-world-cortex'];
    agents.forEach(a => {
      results.push({ name: 'Agent ' + a + ' present', pass: content.includes(a) });
    });

    // Wait for all 4
    results.push({ name: 'Wait for all 4 L1 agents', pass: content.includes('Wait for ALL') });
    ...
    return {
      passed,
      message: passed ? 'All TC-01 checks passed: L1 fires 4-way on every message' : 'Failed: ' + failed.join(', '),
