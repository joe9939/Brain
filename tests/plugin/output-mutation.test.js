function addBrain(p, sid) {
  return p['experimental.chat.system.transform']({sessionID: sid},{system:['# BRAIN ORCHESTRATOR test']});
}
...
module.exports = {
  ...
  run: async () => {
    ...
    try {
      ...
      // Signal gate or G3: throws for write
      await addBrain(plugin, 'om-g3');
      ...
      results.push({ name: 'Write with .env throws (gate or G3)', pass: g3Threw });

      // G5: throws before output mutation
      await addBrain(plugin, 'om-g5');
      ...
      results.push({ name: 'Write with injection throws (gate or G5)', pass: g5Threw });
    } catch (e) {
